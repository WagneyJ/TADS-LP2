...
public class LivroService {
    ...
    public Livro cadastrar(Livro livro){
        if (livro.getTitulo() == null || livro.getTitulo().isEmpty()) 
            throw new RegraDeNegocioException("O titulo é Obrigatório!");
        ...
        if (livro.getAnoPublicacao() <= 0)
            throw new RegraDeNegocioException("O Ano deve ser maior que 0!");

        Optional<Livro> livroJaCadastrado = this.livroRepository.buscarPorIsbn(livro.getIsbn());
        
        if (livroJaCadastrado.isPresent())
            throw new RegraDeNegocioException("Livro com ISBN já cadastrado");
    ...
    public Livro adicionarAutor(Long livroId, String nome){
        Livro livro = this.buscarPorId(livroId);
